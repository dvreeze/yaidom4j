...
public record DescriptionGroup(
    ...
    public static DescriptionGroup parse(AncestryAwareElementApi<?> element) {
        ...
        return new DescriptionGroup(
                ...
                element
                        .childElementStream(e -> e.elementName().equals(new QName(ns, "display-name")))
                        .map(ElementApi::text)
                        .collect(ImmutableList.toImmutableList()),
                element
                        .childElementStream(e -> e.elementName().equals(new QName(ns, "icon")))
                        .map(Icon::parse)
                        .collect(ImmutableList.toImmutableList())
        );
    }
        ...
        return nb.element(
                ...
                ImmutableList.<Node>builder()
                        ...
                        .addAll(
                                displayNames()
                                        .stream()
                                        .map(v -> nb.textElement(nb.name(prefix, "display-name"), v))
                                        .toList()
                        )
                        .addAll(
                                icons()
                                        .stream()
                                        .map(v -> v.toXml(new QName(ns, "icon", prefix)))
                                        .toList()
                        )
                        .build()
        );
